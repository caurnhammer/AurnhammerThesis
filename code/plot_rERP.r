...
generate_topo <- function(
    ...
    # Suppress warning: "The following aesthetics were dropped during
    # statistical transformation: fill"
    # This warning is presumably generated by using fill = EEG

    # Optional legend processing
    if (omit_legend) {
        if (save_legend) {
            suppressWarnings(lgnd <- get_legend(p))        
            ggsave(paste0(file, "_topolegend.pdf"), lgnd,
                    device = cairo_pdf,
                    width = 1.5, height = 4)
        }
    ...
    suppressWarnings(ggsave(paste0(file, "_", cond_man, "_", tw[1], "-", tw[2], ".pdf"),
